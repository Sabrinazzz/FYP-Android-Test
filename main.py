...
time_list_val = df.iloc[0]["Time List"]
survey_number_val = int(df.iloc[0]["Survey Number"])
day_number_val = int(df.iloc[0]["Day Number"])
earliest_survey_time_val = int(df.iloc[0]["Earliest Survey Time"])
latest_survey_time_val = int(df.iloc[0]["Latest Survey Time"])
snooze_counter_val = int(df.iloc[0]["Snooze Counter"])
    ...
    def surveyTaken(self):
        ...
        if self.survey_number < 5: # update survey number until all 6 are complete
            ...
            if self.day_number in range(3,7): # Day: thursday to saturday, no surveys should be triggered
                ...
                self.time_list = []
                self.time_list.append(time_list_val) # when this time comes, the check update function will update the day variable instead of triggering a survey, see line 185
